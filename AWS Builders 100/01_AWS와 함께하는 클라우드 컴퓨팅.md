...
> ##### 중국에서의 확장은 어떻게 되는지?
> - 중국은 전용 콘솔이 따로 있다. 많은 서비스가 있진 않음
> - 유일한 곳이 홍콩밖에 없음
> - 예전부터 **알리바바**를 사용했음
...
> ##### 세대가 바뀌면서 기존에 있던 서버는 사용량이 적어질텐데 이전 세대의 서버는 어떻게 되는지?
> - 한 번 운영서버를 만들면 다시 건들기 조심스럽다.
> - 이전 세대를 사용하는 고객은 분명 있을 것이기 때문에 서버는 계속 놔두어야 한다.
> - 서버 사용량은 떨어지기 때문에 비는 공간을 경매로 내놓고 싼 가격에 판매한다. (Spot Instance - 머신러닝을 사용하는 고객이 많이 사용함)
> - 세대가 바뀔때마다 하이퍼바이저가 바꼈음 (Xen - m3, KVM - m4, Nitro - m5)
>   - `하이퍼바이저란? 호스트 컴퓨터에서 다수의 운영 체제를 동시에 실행하기 위한 논리적 플랫폼`
> - t2, t3는 테스트서버 위주. 가격이 저렴하지만 성능이 좋지 않음. CPU 크레딧(1개의 CPU의 사용률이 100%가 되었을 때, CPU는 Boost상태가 되며 1분동안 1개의 Credit을 소모)을 티어별로 다르게 줌
> - 티어가 올라갈수록 시간당 지급되는 CPU Credit은 많아짐
...
> ##### Lambda@edge
> - 클라우드 프론트에서 제공하는 기능으로 클라우드 프론트 엣지 상에서 람다 함수를 실행함
> - 클라우드 프론트 엣지는 CDN 파일을 전송해주는 캐시 서버로 리전보다 훨씬 다양한 위치에 존재함
> - 람다 엣지는 엣지 로케이션에서 직접 컴퓨팅을 수행하기 때문에 사용자에게 훨씬 더 빠르게 결과를 제공
> - 헤더 조작, SEO, 실시간 이미지 변환 등의 용도로 사용됨
> - [참고하면 좋을 블로그 URL1- 이미지 리사이징 예제](https://afrobambacar.github.io/2018/12/image-resizing-with-lambda-edge.html)
> - [참고하면 좋을 블로그 URL2 - 당근마켓의 람다엣지 적용사례](https://medium.com/daangn/lambda-edge%EB%A1%9C-%EA%B5%AC%ED%98%84%ED%95%98%EB%8A%94-on-the-fly-%EC%9D%B4%EB%AF%B8%EC%A7%80-%EB%A6%AC%EC%82%AC%EC%9D%B4%EC%A7%95-f4e5052d49f3)
...
- AWS Shield Standard 무료 제공 : DDoS 방어 못함 (AWS WAF 서비스 이용하여 DDos 방어)

> ##### Dos와 DDos
> - Dos : 시스템의 구조적 취약점을 공격하여 서비스를 지연, 마비시키는 해킹 공격 (직접적)
> - DDos : 공격자가 여러 대의 컴퓨터를 감염시켜 감염시킨 컴퓨터로 타깃을 동시에 공격하여 서비스를 지연, 마비시키는 공격(간접적)
> - 차이점 : Dos는 공격자가 희생자를 직접 공격하는 방식, DDos는 공격자가 여러 PC를 감염시켜 좀비 PC로 만들고 그 PC들을 통해 희생자를 공격한다.
...
#### Amazon DynamoDB
- **관리형 NoSQL DB 서비스**
- 서버리스 서비스이기 때문에 관리할 서버 필요 없음. 용량에 맞게 테이블을 자동으로 확장/축소하는 Auto Scaling 기능을 이용
- 무제한 처리량, 저장용량 제공
- 모든 데이터가 ssd에 저장되기 때문에 입출력 속도가 빠름
- downtime이 존재하지 않음 (지속적인 서비스 간으)
- HTTP 비연결형 통신으로 데이터를 가져옴. connection pool 관리가 필요없음
- 대표적인 ORM이 없음
#### Amazon ElastiCache
- **인메모리 캐싱 서비스**
- 디스크에 접근하지 않고 메모리로만 모든 처리를 하기 때문에 데이터 저장 및 검색속도가 매우 빠름
- 데이터는 메모리 크기까지만 저장 가능
- 서버의 전원 공급 중단 시 데이터 소멸
- 보통 빠른 처리 속도를 위해 DB에 저자왼 데이터 중 쿼리 결과나 자주 사용되는 데이터를 임시로 저장하는 용도로 사용
